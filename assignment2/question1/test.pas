...
uses scanner, symboltable;
  ...
  begin
    while (sym = LbrakSym) or (sym = PeriodSym) do
      if sym = LbrakSym then
        begin
          write('('); GetSym; expression (y);
          if x.tp^.form <> Arry then Mark ('not an array');
  ...
  procedure ProcedureDecl;
    ...
    begin
      if sym = VarSym then
        begin write('var '); GetSym; IdentList (ParClass, first) end
      else IdentList (VarClass, first);
      if sym = IdentSym then
        begin Find (obj); write(id); GetSym;
            ...
            begin
              ...
              begin
                ...
                while sym = SemicolonSym do
                  begin writeln(';'); GetSym; FPSection end ;
                if sym = RparenSym then
                begin
                  write(')'); GetSym;
                end
  ...
  begin
    if sym = ProgramSym then
      begin
        ...
        if sym = PeriodSym then write('.') else Mark ('. ?');
  ...
  procedure Init;
   begin writeln ('Pascal0 Pretty-Printer');
    OpenScope;
