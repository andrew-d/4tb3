...
implementation
  ...
  function Exponent(var x, y: integer): integer;
  var
      r     : integer;
  begin
    r := 1;
  ...
  begin
    if (x.tp^.form = Int) and (y.tp^.form = Int) then
      if (x.mode = ConstClass) and (y.mode = ConstClass) then
        ...
        if op = ExpSym then begin
          x.a := Exponent(x.a, y.a);
        end
        else if op = PlusSym then x.a := x.a + y.a
        ...
        if op = ExpSym then begin
          { The general algorithm is as follows:
              - Emit loads to registers (call: reg1, reg2).
              - Set reg3 to 1
              - Loop top:
                  - If reg2 mod 2 = 1 then multiply reg3 by reg1
                  - Divide reg2 by 2
                  - If reg2 = 0 then jump to end of loop
                  - reg1 = reg1 * reg1
                  - Jump to start of loop

              <save registers 1 through 6>
              <get inputs to register 1 and 2>
              XOR       3,  3,  0       -- Initialize reg3 to 0
              XOR       5,  5,  0       -- Initialize reg5 and reg6 to 1
              ADDI      5,  5,  1
              XOR       6,  6,  0
              ADDI      6,  6,  1
            loop:
              MODI      4,  2,  2       -- Set temp register to reg2 % 2
              BNE       4,  5,  2       -- Skip the next instruction if (reg2 % 2) != 1
              MUL       6,  6,  1       -- Multiply reg6 by reg1, store in reg6
              DIVI      2,  2,  2       -- Divide reg2 by 2
              BEQ       2,  3,  3       -- If reg2 == 0, then jump to end of loop
              MUL       1,  1,  1       -- reg1 = reg1 * reg1
              BEQ       0,  0, -6       -- Jump to beginning of loop
              <final answer is in reg6>
              <restore registers 1 through 5>
          }

          { Save register 1 through 6 }
          Put(PSHOP, 6, SP, 4);
          Put(PSHOP, 5, SP, 4);
          Put(PSHOP, 4, SP, 4);
          Put(PSHOP, 3, SP, 4);
          ...
          { Set register 3 to 0
            TODO: we can use register 0
          }
          Put(ADDOP, 3, 0, 0);

          { Set registers 5 and 6 to 1 }
          Put(ADDIOP, 5, 0, 1);
          Put(ADDIOP, 6, 0, 1);

          { if reg2 % 2 = 1 then reg6 = reg6 * reg1 }
          Put(MODIOP, 4, 2, 2);
          Put(BNEOP, 4, 5, 2);
          Put(MULOP, 6, 6, 1);
          ...
          { if reg2 = 0 then break out of loop }
          Put(BEQOP, 2, 3, 3);

          { reg1 = reg1 * reg1 }
          Put(MULOP, 1, 1, 1);

          { jump to top of loop, which is 6 instructions up }
          Put(BEQOP, 0, 0, -6);

          { final answer is in register 6 }
          Put(POPOP, 1, SP, 4);
          Put(POPOP, 2, SP, 4);
          Put(POPOP, 3, SP, 4);
          Put(POPOP, 4, SP, 4);
          ...
          { If the destination register is 6, we do nothing, otherwise we
            move from 6 to destination, and then restore 6 }
          if x.r <> 6 then
          begin
            { This taken from PutOp }
            if x.r = 0 then
            begin
              GetReg (x.r);
            end;
        ...
        else if op = PlusSym then PutOp (ADDOP, x, y)
